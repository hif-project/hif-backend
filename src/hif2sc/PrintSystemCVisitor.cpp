...
typedef std::list<Value *> Indexes;
struct AggregateInfo {
    AggregateInfo();
    ~AggregateInfo();
    AggregateInfo(const AggregateInfo &other);
    AggregateInfo &operator=(AggregateInfo other);
    void swap(AggregateInfo &other);
...
AggregateInfo &AggregateInfo::operator=(AggregateInfo other)
{
    swap(other);
    return *this;
}

void AggregateInfo::swap(AggregateInfo &other)
{
...
typedef std::list<AggregateInfo> AggregateInfos;
...
void _fillAggregateAltInfo(Aggregate *agg, AggregateInfos &infos, hif::semantics::ILanguageSemantics *sem)
{
    if (agg->getOthers() != nullptr) {
        AggregateInfos rec;
        Aggregate *recAgg = dynamic_cast<Aggregate *>(agg->getOthers());
        if (recAgg != nullptr) {
            _fillAggregateAltInfo(recAgg, rec, sem);
            for (AggregateInfos::iterator k = rec.begin(); k != rec.end(); ++k) {
    ...
    for (BList<AggregateAlt>::iterator i = agg->alts.begin(); i != agg->alts.end(); ++i) {
        ...
        for (BList<Value>::iterator j = alt->indices.begin(); j != alt->indices.end(); ++j) {
            ...
            AggregateInfos rec;
            Aggregate *recAgg = dynamic_cast<Aggregate *>(alt->getValue());
            if (recAgg != nullptr) {
                _fillAggregateAltInfo(recAgg, rec, sem);
                for (AggregateInfos::iterator k = rec.begin(); k != rec.end(); ++k) {
...
std::string _getNativeConstantModifier(long long value, const bool isSigned, long long bits)
{
    std::stringstream ss;
    if (bits <= 16) {
...
PrintSystemCVisitorOptions &PrintSystemCVisitorOptions::operator=(const PrintSystemCVisitorOptions &opt)
{
    if (this == &opt)
        return *this;
    useResolved             = opt.useResolved;
...
{
    for (ConstTemplateMap::iterator i = list.begin(); i != list.end(); ++i) {
        ObjectList &decls = i->second;
        for (ObjectList::iterator j = decls.begin(); j != decls.end(); ++j) {
            delete *j;
        }
    }
}

PrintSystemCVisitor::BackupOpt PrintSystemCVisitor::_backupVisitMode()
{
...
int PrintSystemCVisitor::visitBit(Bit &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitArray(Array &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitBool(Bool &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitChar(Char &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitEnum(Enum &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitEvent(Event &o)
{
...
int PrintSystemCVisitor::visitInt(Int &o)
{
...
int PrintSystemCVisitor::visitPointer(Pointer &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitReal(Real &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitRecord(Record &o)
{
    _printComment(&o);
    ...
    if (_opt.printFields) {
        ...
        *(_outstream) << "}";
        ...
        if (o.isPacked() && !o.isUnion()) {
            *(_outstream) << "#if (defined _MSC_VER)" << std::endl
        ...
        if (!o.isUnion()) {
            if (o.isPacked()) {
                *(_outstream) << "#if (defined (_MSC_VER)) && ! (defined __GNUC__)" << std::endl
                              << "#pragma pack(push,1)" << std::endl
                              << "#endif" << std::endl;
            }

            if (hif::objectGetLanguage(&o) == hif::c) {
                TypeDef *td = dynamic_cast<TypeDef *>(o.getParent());
...
int PrintSystemCVisitor::visitRecordValue(RecordValue &o)
{
    _printComment(&o);
    ...
    if (!_opt.insideInitList && isDataDeclarationInitialValue) {
        ...
        TypeReference *tr = dynamic_cast<TypeReference *>(dd->getType());
...
int PrintSystemCVisitor::visitRecordValueAlt(RecordValueAlt &o)
{
    _printComment(&o);

    Aggregate *agg = dynamic_cast<Aggregate *>(o.getValue());
...
int PrintSystemCVisitor::visitReference(Reference &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitString(String &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitSystem(hif::System &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitTypeReference(TypeReference &o)
{
    _printComment(&o);

    if (_isNeededTypename(&o)) {
        *(_outstream) << "typename " << endl;
    }
    ...
    *(_outstream) << o.getName();
    ...
    if (dynamic_cast<TypeDef *>(tdect) != nullptr) {
        TypeDef *td = static_cast<TypeDef *>(tdect);
...
int PrintSystemCVisitor::visitAggregate(Aggregate &o)
{
    _printComment(&o);

    if (o.alts.empty()) {
        Bitvector *bv      = dynamic_cast<Bitvector *>(hif::semantics::getSemanticType(&o, _sem));
        Bit *othersType    = dynamic_cast<Bit *>(hif::semantics::getSemanticType(o.getOthers(), _sem));
        String *stringType = dynamic_cast<String *>(hif::semantics::getSemanticType(&o, _sem));
        if (bv != nullptr && othersType != nullptr) {
            bv->acceptVisitor(*this);

            // Just smoking&frac
            BitValue *bval = dynamic_cast<BitValue *>(o.getOthers());
            if (bval != nullptr) {
...
int PrintSystemCVisitor::visitAggregateAlt(AggregateAlt &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitBitvector(Bitvector &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitBitvectorValue(BitvectorValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitBoolValue(BoolValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitCast(Cast &o)
{
    _printComment(&o);
    ...
    if (dynamic_cast<Time *>(o.getType()) != nullptr) {
        RecordValue *r = dynamic_cast<RecordValue *>(o.getValue());
...
int PrintSystemCVisitor::visitCharValue(CharValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitExpression(Expression &o)
{
    _printComment(&o);
    ...
    case op_pow: {
        if (dynamic_cast<IntValue *>(o.getValue1()) != nullptr) {
            IntValue *intval_o   = static_cast<IntValue *>(o.getValue1());
...
int PrintSystemCVisitor::visitFunctionCall(FunctionCall &o) { return _printCall(o); }

int PrintSystemCVisitor::visitFieldReference(FieldReference &o)
{
    _printComment(&o);
    ...
    if (dynamic_cast<Instance *>(o.getPrefix()) != nullptr) {
        ...
        if (dynamic_cast<Library *>(t) != nullptr) {
            Library *lib = static_cast<Library *>(t);
...
int PrintSystemCVisitor::visitIntValue(IntValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitMember(Member &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitGlobalAction(GlobalAction &o)
{
    if (_opt.printImplementation)
        return 0;
...
int PrintSystemCVisitor::visitIdentifier(Identifier &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitParameterAssign(ParameterAssign &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitRealValue(RealValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitStringValue(StringValue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitTypeTPAssign(TypeTPAssign &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitValueTPAssign(ValueTPAssign &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitWhen(When &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitWhenAlt(WhenAlt &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitWith(With &o) { messageError("With should be mapped to When", &o, _sem); }

int PrintSystemCVisitor::visitWithAlt(WithAlt &o)
{
...
int PrintSystemCVisitor::visitContents(Contents &o)
{
    _printComment(&o);
    _printAdditionalKeywords(&o);
    ...
    if (!_opt.printImplementation) {
        ...
        if (_opt.publicDecl) {
            for (BList<Declaration>::iterator it = o.declarations.begin(); it != o.declarations.end(); ++it) {
                ...
                SubProgram *sp = dynamic_cast<SubProgram *>(*it);
    ...
    // managed here, since it depends on the presence of TPs and cannot be general.
        ...
        for (BList<Declaration>::iterator it = o.declarations.begin(); it != o.declarations.end(); ++it) {
            ...
            DataDeclaration *ddecl = dynamic_cast<DataDeclaration *>(*it);
...
int PrintSystemCVisitor::visitConst(Const &o)
{
    if (!_opt.printImplementation) {
        _printComment(&o);
    ...
    if (!isInModule || (isInModule && !o.isInstance())
        ...
        SubProgram *sp     = hif::getNearestParent<SubProgram>(&o);
        ...
        *(_outstream) << "const ";
        ...
        if (dynamic_cast<ReferencedType *>(o.getType()) != nullptr) {
            ...
            if (_isNeededTypename(static_cast<ReferencedType *>(o.getType())))
                (*_outstream) << "typename ";
        ...
        ReferencedType *rt = dynamic_cast<ReferencedType *>(o.getType());
...
int PrintSystemCVisitor::visitEntity(Entity &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitField(Field &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitFunction(Function &o)
{
    if (o.isStandard())
        return 0;
...
int PrintSystemCVisitor::visitInstance(Instance &o)
{
    _printComment(&o);
    ...
    if (!_opt.printImplementation) {
        ...
        *(_outstream) << o.getName();
        ...
        if (isPointer) {
            ViewReference *vrI = dynamic_cast<ViewReference *>(o.getReferencedType());
            TypeReference *trI = dynamic_cast<TypeReference *>(o.getReferencedType());
            Library *libI      = dynamic_cast<Library *>(o.getReferencedType());
            string instName;
            if (trI != nullptr) {
        ...
        if (o.getValue() != nullptr) {
            FunctionCall *fc = dynamic_cast<FunctionCall *>(o.getValue());
            messageAssert(
...
int PrintSystemCVisitor::visitLibrary(hif::Library &o)
{
    if (o.isStandard()) {
        return 1;
    }
...
int PrintSystemCVisitor::visitLibraryDef(hif::LibraryDef &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitParameter(Parameter &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
    ...
    Reference *ref   = dynamic_cast<Reference *>(paramType);
    const bool isRef = (ref != nullptr);
    bool isArray     = false;
    if (isRef) {
        Array *refType = dynamic_cast<Array *>(hif::semantics::getBaseType(ref->getType(), false, _sem));
        isArray        = refType != nullptr;
    } else {
        Array *refType = dynamic_cast<Array *>(paramType);
        isArray        = refType != nullptr;
    }
...
int PrintSystemCVisitor::visitPort(Port &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
    ...
    if (_opt.printImplementation) {
        if (_opt.insideInitList) {
            ...
            *(_outstream) << ")";
            ...
            if (_opt.printInitVal && o.getValue() && (o.getDirection() == dir_out || o.getDirection() == dir_inout)) {
                std::list<std::string> indexes;
    ...
    } else // if (! _opt.printImplementation)
    {
        ...
        Array *array        = dynamic_cast<Array *>(baseType);
        Record *record      = dynamic_cast<Record *>(baseType);
...
int PrintSystemCVisitor::visitProcedure(Procedure &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
    ...
    if (!_opt.printImplementation) {
        if (du != nullptr && _isCppDestructor(&o)) {
            ...
            _printSubProgramDeclaration(o, dtorName.c_str());
            _outstream->newLine();
...
int PrintSystemCVisitor::visitSignal(Signal &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitStateTable(StateTable &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitTime(Time &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitTimeValue(TimeValue &o)
{
    _printComment(&o);
    *(_outstream) << "sc_core::sc_time( ";
    *(_outstream) << o.getValue();
...
int PrintSystemCVisitor::visitTypeDef(TypeDef &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
    ...
    if (!o.templateParameters.empty()) {
        ...
        *(_outstream) << ";";
        ...
        Record *robj = dynamic_cast<Record *>(o.getType());
        if (robj == nullptr)
            *(_outstream) << "typedef ";
...
int PrintSystemCVisitor::visitTypeTP(TypeTP &o)
{
    _printComment(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitUnsigned(Unsigned &o)
{
    messageDebugAssert(false, "Unsigned not supported yet.", &o, _sem);
    messageError("Unsigned not supported yet.", &o, _sem);
}

int PrintSystemCVisitor::visitValueStatement(ValueStatement &o)
{
    o.getValue()->acceptVisitor(*this);
    return 0;
}

int PrintSystemCVisitor::visitValueTP(ValueTP &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitVariable(Variable &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitAssign(Assign &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitIf(If &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitIfAlt(IfAlt &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitIfGenerate(IfGenerate &o)
{
    messageError("If generate statements should be managed in VHDL frontend.", &o, _sem);
}

int PrintSystemCVisitor::visitBreak(Break &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitFile(File & /*o*/)
{
    *(_outstream) << "FILE *";
    return 1;
}

int PrintSystemCVisitor::visitFor(For &o)
{
...
int PrintSystemCVisitor::visitForGenerate(ForGenerate &o)
{
...
int PrintSystemCVisitor::visitContinue(Continue &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitProcedureCall(ProcedureCall &o) { return _printCall(o); }

int PrintSystemCVisitor::visitReturn(Return &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitState(State &o)
{
    _printComment(&o);
    _printDefineMacros(&o);
    _printAdditionalKeywords(&o);
...
int PrintSystemCVisitor::visitSwitch(Switch &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitSwitchAlt(SwitchAlt &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitWait(Wait &o)
{
    _printComment(&o);
...
int PrintSystemCVisitor::visitWhile(While &o)
{
    _printComment(&o);
...
void PrintSystemCVisitor::_printSensitivityLoop(
    ...
    std::list<std::string> &indexes)
{
    Array *ao = dynamic_cast<Array *>(nameType);
    if (ao != nullptr) {
        Range *aoSpan = ao->getSpan();
        if (dynamic_cast<Slice *>(name) != nullptr) {
            Slice *slice = static_cast<Slice *>(name);
        ...
        *(_outstream) << "sensitive << ";
        ...
        for (std::list<std::string>::iterator it = indexes.begin(); it != indexes.end(); ++it) {
            *(_outstream) << "[" << (*it) << "]";
        }
...
void PrintSystemCVisitor::_printSensitivitySuffix(Value *name, bool isPos, bool isNeg)
{
    ...
    Signal *sigDecl   = dynamic_cast<Signal *>(deco);
    ...
    ViewReference *vr = dynamic_cast<ViewReference *>(exprType);
    bool isClock      = false;
    if (vr != nullptr) {
...
PrintSystemCVisitor::BackupOpt &PrintSystemCVisitor::BackupOpt::operator=(const BackupOpt &b)
{
    if (this == &b)
        return *this;
    _opt             = b._opt;
...
bool PrintSystemCVisitor::_isSystemCResolved(Type *type)
{
...
bool PrintSystemCVisitor::_isFullySpecifiedArrayConst(Const *c)
{
    Type *bt = hif::semantics::getBaseType(c->getType(), false, _sem);
    Array *t = dynamic_cast<Array *>(bt);
    if (t == nullptr)
        return false;
...
Type *PrintSystemCVisitor::_needIndividualInit(DataDeclaration *ddo)
{
...
void PrintSystemCVisitor::_printIndividualInit(DataDeclaration *ddo, Array *baseT)
{
    ...
    // Otherwise, trust it.
    Aggregate *agg = dynamic_cast<Aggregate *>(ddo->getValue());
    if (agg == nullptr) {
        *(_outstream) << ddo->getName();
    ...
    for (AggregateInfos::iterator i = infos.begin(); i != infos.end(); ++i) {
        AggregateInfo &info = *i;
        ...
        for (Indexes::iterator j = info.indexes.begin(); j != info.indexes.end(); ++j) {
            Value *ind      = *j;
            Range *arrRange = nullptr;

            if (dynamic_cast<Aggregate *>(ind) != nullptr) {
                Aggregate *subAggr = static_cast<Aggregate *>(ind);
                ...
                Array *arr = dynamic_cast<Array *>(hif::semantics::getBaseType(subAggrType, false, _sem));
                ...
                Range *rangeInd = static_cast<Range *>(ind);
                arrRange        = rangeInd;
            }
        ...
        // 2- print for body
        RecordValue *rv = dynamic_cast<RecordValue *>(info.value);
        std::string varName;
        ...
        *(_outstream) << ";";
        ...
        // 3- closing for loops
        for (IndexNames::iterator j = forIndexes.begin(); j != forIndexes.end(); ++j) {
            _outstream->unindent();
            *(_outstream) << "}";
            _outstream->newLine();
        }
...
bool PrintSystemCVisitor::_printTLMInit(DataDeclaration *o)
{
    ...
    ViewReference *vr            = dynamic_cast<ViewReference *>(o->getType());
            ...
            *(_outstream) << o->getName() << "(*this);";
            ...
            if (o->getValue() != nullptr) {
                ...
                if (dynamic_cast<Cast *>(valueToPrint) != nullptr) {
                    Cast *cc     = static_cast<Cast *>(valueToPrint);
...
void PrintSystemCVisitor::_printRecordClasslikeMethods(hif::Record *obj)
{
    if (objectGetLanguage(obj) == hif::c)
        return;
    Declaration *decl = hif::getNearestParent<Declaration>(obj);
    ...
    *(_outstream) << " ):";
    ...
    std::list<std::string>::iterator pars(params.begin());
...
std::string PrintSystemCVisitor::_printAggregateRecordValue(RecordValue *rv, DataDeclaration *ddo)
{
    // 1- Getting recordvalue type name
    Type *type        = hif::semantics::getSemanticType(rv, _sem);
    TypeReference *tr = dynamic_cast<TypeReference *>(type);
    if (tr == nullptr) {
...
bool PrintSystemCVisitor::_initListInitializationSignalPort(DataDeclaration *dd)
{
    Signal *sig = dynamic_cast<Signal *>(dd);
    ...
    *(_outstream) << "\"" << dd->getName() << "\"";
    ...
    if (isWrapper) {
        FunctionCall *ctor = dynamic_cast<FunctionCall *>(dd->getValue());
...
bool PrintSystemCVisitor::_printNormalInit(DataDeclaration *o)
{
    // "Unroll" of Pointers and ViewReferences
    Pointer *po         = dynamic_cast<Pointer *>(o->getType());
    ViewReference *vref = dynamic_cast<ViewReference *>(o->getType());
    if (vref == nullptr && po != nullptr && dynamic_cast<ViewReference *>(po->getType()) != nullptr) {
        vref = static_cast<ViewReference *>(po->getType());
    }
    ...
    } else // may be inside constructor body or inside other scopes.
    {
        ...
        ViewReference *vr = dynamic_cast<ViewReference *>(
        ...
        if (dynamic_cast<Const *>(o) != nullptr) {
            ...
            return true;
        } else if (isSignal && static_cast<Signal *>(o)->isWrapper())
            return false;
        ...
        if (_opt.printType) {
            if (isSignal) {
                _printSignalTypeAndName(static_cast<Signal *>(o));
                *(_outstream) << "(\"";
                *(_outstream) << o->getName();
                *(_outstream) << "\");";
                return true;
        ...
        if (dynamic_cast<RecordValue *>(o->getValue()) != nullptr) {
            ...
            *(_outstream) << ";";
            return true;
        } else if (_isCppConstructor(o->getValue()) && dynamic_cast<Pointer *>(o->getType()) == nullptr) {
...
void PrintSystemCVisitor::_visitType(Type *type, bool different_management)
{
    ...
    if (dynamic_cast<Array *>(type) != nullptr) {
        _visitTypeArray(static_cast<Array *>(type), different_management);
        return;
...
bool PrintSystemCVisitor::_isNeededTypename(ReferencedType *refType)
{
    Library *lib = dynamic_cast<Library *>(refType);
    if (lib != nullptr)
        return false;

    // Allowed ReferencedTypes
    ViewReference *vR = dynamic_cast<ViewReference *>(refType);
    TypeReference *tR = dynamic_cast<TypeReference *>(refType);

    // Scope nodes that own TPs
    View *viewP          = hif::getNearestParent<View>(refType->getParent());
    SubProgram *subprogP = hif::getNearestParent<SubProgram>(refType->getParent());
    ...
    for (std::list<Object *>::iterator j = ids.begin(); j != ids.end(); ++j) {
        Identifier *id                    = static_cast<Identifier *>(*j);
...
void PrintSystemCVisitor::_printCppConstructor(FunctionCall &o)
{
    ...
    DataDeclaration *dd  = dynamic_cast<DataDeclaration *>(o.getParent());
    Instance *pp         = dynamic_cast<Instance *>(o.getParent());
...
void PrintSystemCVisitor::_printCppDestructor(ProcedureCall &o)
{
    ...
    Pointer *pointer     = dynamic_cast<Pointer *>(baseType);
    ...
    ReferencedType *refType = dynamic_cast<ReferencedType *>(typeToPrint);
    if (refType == nullptr) {
        *(_outstream) << "~";
...
bool PrintSystemCVisitor::_printNativeFunctionCall_new(FunctionCall &o)
{
    ...
    *(_outstream) << "new ";
    ...
    if (o.parameterAssigns.size() == 1 && o.templateParameterAssigns.empty() &&
        ...
        FunctionCall *ctor = dynamic_cast<FunctionCall *>(o.parameterAssigns.front()->getValue());
        messageDebugAssert(ctor != nullptr, "Expected constructor inside new", &o, _sem);

        ctor->acceptVisitor(*this);

        Instance *inst = dynamic_cast<Instance *>(ctor->getInstance());
        ...
        messageAssert(
            ...
            &o, _sem);
        TypeTPAssign *ttpa = static_cast<TypeTPAssign *>(o.templateParameterAssigns.front());
        ...
        messageAssert(o.parameterAssigns.size() >= 1, "Expected at least one parameter parameters", &o, _sem);
        messageAssert(
...
bool PrintSystemCVisitor::_printNativeFunctionCall_malloc(FunctionCall &o)
{
...
void PrintSystemCVisitor::_printNativeProcedureCall(ProcedureCall &o)
{
    if (objectMatchName(&o, "delete")) {
        ...
        *(_outstream) << "free ";
        ...
        // placement new: first parameter is the address
        messageAssert(o.parameterAssigns.size() >= 1, "Expected at least one parameter", &o, _sem);
...
void PrintSystemCVisitor::_printInstanceBindingStatements(Instance *inst, Object *o, const bool useArrow)
{
    ...
    Assign *parentAss      = dynamic_cast<Assign *>(o->getParent());
    const bool isAssignRhs = parentAss != nullptr && parentAss->getRightHandSide() == o;

    Variable *var = dynamic_cast<Variable *>(o->getParent());
    Identifier id;
...
bool PrintSystemCVisitor::_isStatement(Object *obj)
{
    if (dynamic_cast<FunctionCall *>(obj) != nullptr) {
        FunctionCall *fc = static_cast<FunctionCall *>(obj);
        return (dynamic_cast<Action *>(fc->getParent()) != nullptr);
    } else if (dynamic_cast<Action *>(obj) != nullptr) {
...
bool PrintSystemCVisitor::_isTLMComponent(DataDeclaration *obj)
{
    // Check the implementation language.
    if (!checkLanguage(obj, hif::tlm))
        return false;

    ViewReference *vr = dynamic_cast<ViewReference *>(obj->getType());
    if ((vr != nullptr && vr->getDesignUnit() == std::string("tlm_target_socket")) ||
...
bool PrintSystemCVisitor::_isCppConstructor(Object *obj)
{
    // Call from Function may have not set name.
    Function *f = dynamic_cast<Function *>(obj);
    if (f != nullptr)
        return (f->getName() == hif::NameTable::getInstance()->hifConstructor());

    FunctionCall *fc = dynamic_cast<FunctionCall *>(obj);
    if (fc != nullptr)
...
bool PrintSystemCVisitor::_isCppDestructor(Object *obj)
{
    return hif::objectGetName(obj) == hif::NameTable::getInstance()->hifDestructor();
}

bool PrintSystemCVisitor::_isNativeHifFunction(Object *obj)
{
    if (dynamic_cast<FunctionCall *>(obj) == nullptr)
        return false;
...
bool PrintSystemCVisitor::_isNativeHifProcedure(Object *obj)
{
...
template <typename T> bool PrintSystemCVisitor::_needTemplateAsQualifier(T *o)
{
    ...
    bool found  = false;
    Scope *curr = hif::getNearestParent<Scope>(o);

    while (curr != nullptr) {
        View *v        = dynamic_cast<View *>(curr);
        SubProgram *sp = dynamic_cast<SubProgram *>(curr);
        TypeDef *td    = dynamic_cast<TypeDef *>(curr);

        if (v != nullptr && !v->templateParameters.empty()) {
            found = true;
            break;
        } else if (sp != nullptr && !sp->templateParameters.empty()) {
            found = true;
            break;
...
bool PrintSystemCVisitor::_containsOnlyIndependentComponents(LibraryDef &o)
{
    for (BList<Declaration>::iterator it = o.declarations.begin(); it != o.declarations.end(); ++it) {
...
bool PrintSystemCVisitor::_isTemplateInstance(Value *o)
{
    Type *t = hif::semantics::getSemanticType(o, _sem);
    if (t == nullptr)
        return false;

    ViewReference *vr = dynamic_cast<ViewReference *>(t);
...
bool PrintSystemCVisitor::_printTemplateParameters(BList<Declaration> &temp_params, bool typed, bool init)
{
    BackupOpt backup         = _backupVisitMode();
...
void PrintSystemCVisitor::_printNotCompileTimeTemplates(Object *o, PrintSystemCVisitor::ConstTemplateContext c)
{
    ...
    const bool hasDefaultParents =
        dynamic_cast<View *>(o) != nullptr && (isLangWithParents || !static_cast<View *>(o)->inheritances.empty());
    ...
    case CONST_TEMPL_CTOR_INIT_LIST: {
        ...
        if (!isLangWithParents && !list.empty() && dynamic_cast<View *>(o) != nullptr &&
            !static_cast<View *>(o)->getContents()->declarations.empty())
            *(_outstream) << ",\n";
        break;
    }
...
bool PrintSystemCVisitor::_needWrapParen(Object *v)
{
    ...
    if (prec == prec_concat && parentPrec != prec_concat)
        return true;
    if (parentPrec == prec_member && static_cast<Member *>(parent)->getPrefix() != v)
        return false;
    if (prec >= prec_access && prec <= prec_scope)
        return false;
    if (prec == parentPrec && dynamic_cast<Expression *>(parent) != nullptr &&
        static_cast<Expression *>(parent)->getValue2() == v) {
...
PrintSystemCVisitor::OperatorPecedenceEnum PrintSystemCVisitor::_getOperatorPrecedence(Object *v)
{
    if (v == nullptr)
        return prec_min;

    if (dynamic_cast<Expression *>(v) != nullptr) {
        Expression *e = static_cast<Expression *>(v);
        ...
        return prec_min;
    } else if (dynamic_cast<Member *>(v) != nullptr) {
        return prec_member;
    } else if (
...
int PrintSystemCVisitor::_printFullTP(BList<Declaration> &temp_params)
{
    return _printTemplateParameters(temp_params, true, true);
}

bool PrintSystemCVisitor::_printTypedTP(BList<Declaration> &temp_params)
{
    return _printTemplateParameters(temp_params, true, false);
}

int PrintSystemCVisitor::_printUntypedTP(BList<Declaration> &temp_params)
{
    return _printTemplateParameters(temp_params, false, false);
}
...
bool PrintSystemCVisitor::_isAMSPort(Port *o)
{
    Type *portType = o->getType();

    // Recursively resolve Pointers and References - just for checking
    Pointer *p   = dynamic_cast<Pointer *>(portType);
    Reference *r = dynamic_cast<Reference *>(portType);
    do {
        if (p != nullptr) {
            portType = p->getType();
    ...
    ViewReference *vr = dynamic_cast<ViewReference *>(portType);
    if (vr == nullptr)
        return false;
    ...
    LibraryDef *amsLib = hif::getNearestParent<LibraryDef>(v);
    if (amsLib == nullptr || !objectMatchName(amsLib, "systemc_sca_eln"))
        return false;
...
void PrintSystemCVisitor::_printImplementationBegin(Object *obj)
{
    ...
    if (!_opt.printImplementation_ihh) {
        ...
        StringValue *tex   = dynamic_cast<StringValue *>(props);
...
void PrintSystemCVisitor::_printHeaderGuardEnd(Object &o, const std::string &suggestedName)
{
    if (!_opt.printImplementation && (ownTemplate(&o, false) || ownTemplate(&o, true))) {
        ...
        if (dynamic_cast<DesignUnit *>(&o) != nullptr) {
            name = static_cast<DesignUnit *>(&o)->getName();
        } else if (dynamic_cast<LibraryDef *>(&o) != nullptr) {
            name     = static_cast<LibraryDef *>(&o)->getName();
            isLibDef = true;
        }
...
void PrintSystemCVisitor::_printModuleContents_I(View *view)
{
    ...
    if (c != nullptr && (!_opt.printImplementation_ihh || ownTemplate(view, false))) {
        DesignUnit *du = hif::getNearestParent<DesignUnit>(view);
    ...
    for (BList<Declaration>::iterator it(view->declarations.begin()); it != view->declarations.end(); ++it) {
        ...
        SubProgram *sub = dynamic_cast<SubProgram *>(*it);
    ...
    const std::string &includeName)
{
    ...
    for (BList<Declaration>::iterator i = ld->declarations.begin(); i != ld->declarations.end(); ++i) {
        ...
        LibraryDef *libDef = dynamic_cast<LibraryDef *>(decl);
...
bool PrintSystemCVisitor::_needInitializationList(View *view)
{
    ...
    if (c != nullptr) {
        for (BList<Declaration>::iterator it = c->declarations.begin(); it != c->declarations.end(); ++it) {
            ...
            DataDeclaration *ddo = dynamic_cast<DataDeclaration *>(*it);
            if (ddo == nullptr)
                continue;

            // Individual initialization.
            if (_needIndividualInit(ddo))
                continue;

            // Member constants are initialized in constructor initialization list.
            if (dynamic_cast<Const *>(*it) != nullptr && static_cast<Const *>(*it)->isInstance() &&
                !_isFullySpecifiedArrayConst(static_cast<Const *>(*it)))
                return true;
...
int PrintSystemCVisitor::_printInitializationList(View *view)
{
...
int PrintSystemCVisitor::_printOtherInitializations(View *view)
{
    BackupOpt backup           = _backupVisitMode();
    _opt.printInitVal          = true;
    _opt.insideInitList        = false;
...
void PrintSystemCVisitor::_printConstants(BList<Declaration> &declarations, Object *scope, const bool onlyDefines)
{
    ...
    const bool templateModule =
        (isInModule && !static_cast<DesignUnit *>(scope)->views.front()->templateParameters.empty());
    ...
    for (BList<Declaration>::iterator it = declarations.begin(); it != declarations.end(); ++it) {
        ...
        if (dynamic_cast<DesignUnit *>(*it) != nullptr) {
            ...
            if (it != declarations.begin()) {
                ...
                --tmp;
                DesignUnit *prevDU = dynamic_cast<DesignUnit *>(*tmp);
                prevIsTDefDU       = prevDU != nullptr && prevDU->checkProperty(PROPERTY_TYPDEF_DESIGN_UNIT);
            }
            if (it != declarations.end()) {
                BList<Declaration>::iterator tmp(it);
                ++tmp;
                DesignUnit *nextDU = dynamic_cast<DesignUnit *>(*tmp);
        ...
        else if (dynamic_cast<Const *>(*it) != nullptr)
            continue;
...
std::string PrintSystemCVisitor::_calculateInclude(Object *where, Library *lib)
{
    if (lib->isStandard())
        return "";
...
void PrintSystemCVisitor::_printIncludes(BList<Library> &list, DesignUnit *du)
{
    ...
    for (Includes::iterator i = system.begin(); i != system.end(); ++i) {
        const std::string &inc = *i;
        *_outstream << inc << std::endl;
    }
    for (Includes::iterator i = local.begin(); i != local.end(); ++i) {
        const std::string &inc = *i;
        *_outstream << inc << std::endl;
    }
...
PrintSystemCVisitor::PrintListOpt::PrintListOpt(const PrintListOpt &other)
    : _mandatoryParen(other._mandatoryParen)
    , _mandatoryNoParen(other._mandatoryNoParen)
    , _angularParen(other._angularParen)
    , _curlyParen(other._curlyParen)
    , _breakLine(other._breakLine)
{
}

PrintSystemCVisitor::PrintListOpt &PrintSystemCVisitor::PrintListOpt::operator=(const PrintListOpt &other)
{
    if (this == &other)
        return *this;
...
void PrintSystemCVisitor::_printDefineMacros(Object *o)
{
    if (o->checkProperty(PROPERTY_REQUIRED_MACRO)) {
        ...
        StringValue *def = dynamic_cast<StringValue *>(pp);
...
void PrintSystemCVisitor::_printAdditionalKeywords(Declaration *o)
{
    ...
    if (dynamic_cast<Variable *>(o) != nullptr && _opt.printImplementation)
        return;
    Declaration::KeywordList::iterator it = o->getAdditionalKeywordsBeginIterator();
...
void PrintSystemCVisitor::_printInitialize(Port &o, Type *portType, std::list<std::string> &indexes, const bool isAMS)
{
    ...
    Array *arrT = dynamic_cast<Array *>(baseT);
    if (arrT == nullptr) {
        // Base cases.
        if (indexes.empty()) {
            // Simple initialize.
...
void PrintSystemCVisitor::_printPortBinding(
    ...
    std::list<std::string> &indexes)
{
    Array *arrT = dynamic_cast<Array *>(t);
    if (arrT == nullptr) {
        // Base cases.
        if (indexes.empty()) {
            // Simple initialize.
...
std::string PrintSystemCVisitor::_capitalize(const char *str)
{
    std::string ret(str);
    bool fresh = false;
    for (size_t i = 0; i < ret.length(); ++i) {
        ret[i] = static_cast<char>(toupper(ret[i]));
        if (!::isalnum(ret[i]) && ret[i] != '_') {
            ret[i] = '_';
            fresh  = true;
        }
    }
...
bool PrintSystemCVisitor::_isInt32Type(Expression *e)
{
...
bool PrintSystemCVisitor::_mayBeAmbiguous(IntValue *o)
{
    Object *lastValue      = o;
    Expression *parentExpr = dynamic_cast<Expression *>(lastValue->getParent());
    ...
    Range *rr = dynamic_cast<Range *>(parent);
    if (rr != nullptr)
        return false;

    Assign *aa = dynamic_cast<Assign *>(parent);
    if (aa != nullptr)
        return (aa->getRightHandSide() != lastValue);

    Return *ret = dynamic_cast<Return *>(parent);
    if (ret != nullptr)
        return false;

    Aggregate *agg = dynamic_cast<Aggregate *>(parent); // ref. design: itc99/b12
    if (agg != nullptr)
        return false;

    Member *mm = dynamic_cast<Member *>(parent);
    if (mm != nullptr)
        return (mm->getIndex() != lastValue);

    DataDeclaration *dd = dynamic_cast<DataDeclaration *>(parent);
    if (dd != nullptr)
    ...
    ParameterAssign *pp = dynamic_cast<ParameterAssign *>(parent);
    if (pp != nullptr) {
    ...
    ValueTPAssign *vtpa = dynamic_cast<ValueTPAssign *>(parent);
    if (vtpa != nullptr) {
...
bool PrintSystemCVisitor::_mayBeAmbiguous(StringValue *o)
{
    ...
    FunctionCall *fc = dynamic_cast<FunctionCall *>(parent);
    if (fc != nullptr)
        return true;

    ProcedureCall *pc = dynamic_cast<ProcedureCall *>(parent);
    if (pc != nullptr)
        return true;
...
template <typename T> int PrintSystemCVisitor::_printCall(T &o)
{
    _printComment(&o);
    ...
    if (_isNativeHifProcedure(&o)) {
        ProcedureCall *fc = dynamic_cast<ProcedureCall *>(&o);
        messageAssert(fc != nullptr, "Unexpected object.", &o, _sem);
    ...
    if (_isCppDestructor(&o)) {
        ProcedureCall *pc = dynamic_cast<ProcedureCall *>(&o);
    ...
    if (o.getInstance() != nullptr) {
        ...
        if (dec != nullptr && dec->getKind() == SubProgram::STATIC) {
            ...
            Library *lib        = dynamic_cast<Library *>(t);
